...
// ║  Payment se related saare actions                   ║
// ║  Functions:                                         ║
// ║    verifyPayment     — UTR se verify karna          ║
// ║    simulatePayment   — Dev mode test                ║
// ║    submitProof       — User proof submit karta hai  ║
// ║    adminCallback     — Telegram webhook handler     ║
// ║    getPaymentResult  — Frontend polling endpoint    ║
// ╚══════════════════════════════════════════════════════╝
...
// const TELE_BASE = `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}`;

// // ─── Existing: verify by UTR ──────────────────────────────────────────────────
// export const verifyPayment = async (req, res) => {
//   try {
//     const { txnId, utrNumber } = req.body;
...
// // ─── Existing: simulate payment (DEMO only) ───────────────────────────────────
// export const simulatePayment = async (req, res) => {
//   try {
//     const { txnId } = req.body;
...
// // ─── NEW: User submits payment proof ─────────────────────────────────────────
// export const submitProof = async (req, res) => {
//   try {
//     const { txnId, utrNumber, userName } = req.body;
...
//     if (!screenshotFile && !utrNumber?.trim()) {
//       return res.status(400).json({
//         error: "Please provide screenshot or UTR number",
//       });
//     }

//     // Fetch transaction
//     const [rows] = await pool.query(
//       `SELECT t.*, b.upi_id, b.account_name, b.account_no, b.ifsc_code, b.bank_name
//        FROM transactions t
//        JOIN bank_accounts b ON t.bank_id = b.id
//        WHERE t.id=?`,
//       [txnId]
...
//     // Allow re-submission if still pending or submitted
//     if (!["pending", "submitted"].includes(txn.status)) {
//       return res.status(400).json({
//         error: `Transaction already ${txn.status}`,
//         status: txn.status,
...
//     await pool.query(
//       `UPDATE transactions
//        SET status='submitted', utr_number=?, submitted_at=NOW()
//        WHERE id=?`,
//       [utrNumber?.trim() || null, txnId]
//     );
...
//     return res.json({
//       success: true,
//       message: "Proof submitted. Admin will verify shortly.",
//     });
//   } catch (err) {
//     console.error("submitProof error:", err);
//     res.status(500).json({ error: err.message });
...
// export const adminCallback = async (req, res) => {
//   try {
//     // Delegate all logic to the bot handler (handles both buttons and remark messages)
//     await handleTelegramUpdate(req.body);
...
//     return res.json({ status, remark: admin_remark || null, coins });
//   } catch (err) {
//     res.status(500).json({ error: err.message });
//   }
// };