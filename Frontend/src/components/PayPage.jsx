...
import { useState, useEffect, useRef, useCallback } from 'react'
// import { QRCode } from 'react-qr-code' // ❌ Remove – we'll use admin's QR image

// ─── Config ───────────────────────────────────────────────────────────────────
const API = import.meta.env.VITE_BACKEND_PAYMENT_URL
const APIMAIN = import.meta.env.VITE_BACKEND_MAIN_URL
const RESULT_POLL_MS = 4000   // submitted ke baad har 4s poll karo
...
export default function PayPage({ txnId }) {
    ...
    // ── Start result polling ONLY after submission ────────────────────────────
    const startResultPolling = useCallback(() => {
        if (resultPollRef.current) return
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  RENDER (same as before, only QR tab changed)
    // ─────────────────────────────────────────────────────────────────────────
    if (loading) return (
        <Shell>
            <Center>
    ...
    return (
        <Shell>
            ...
            {tab === 'qr' && (
                <div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '22px' }}>
                        {/* Show QR image uploaded by admin (if available) */}
                        {txn?.qr_image ? (
                            <div style={{ padding: '10px', background: '#ffffff', borderRadius: '18px', border: '1px solid #1e293b', boxShadow: '0 0 40px #38bdf812' }}>
                                <img
                                    ...
                                    onError={(e) => {
                                        // fallback: if image fails to load, show a message
                                        e.target.onerror = null;
                                        e.target.style.display = 'none';
                                        e.target.parentElement.innerText = 'QR image failed to load. Please use UPI ID.';
                                    }}
                                />
                            </div>
                        ) : (
                            <div style={{ padding: '20px', background: '#1e293b', borderRadius: '18px', color: '#94a3b8', fontSize: '12px', textAlign: 'center' }}>
                                QR code not available. Please copy UPI ID below.
                                {/* <img style={{ width: "300px" }} src={`${APIMAIN} /${txn?.qr_image}`} alt="" /> */}
                            </div>
                        )}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '12px' }}>
                            <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#22c55e', animation: 'gpPulse 1.5s infinite' }} />
                    ...
                    {/* UPI Apps buttons (optional, can keep as is) */}
                    {/* ... (keep the UPI_APPS section if you still want it) */}
                </div>
            )}
...
// ─── Shell, Center, helpers (unchanged) ─────────────────────────────────────
function Shell({ children }) {
    return (
        <>
            <style>{`
